...
c.showControls&&(0==b?n.find("a.prev").hide():b==L-1?n.find("a.next").hide():n.find("a.next, a.prev").show());a.onBeforeShow.call(h,m);p.clearQueue().stop().css({width:navigator.userAgent.match(/opera/i)?"1%":"0%"});f["min-width"]=e.width();if(a.expose&&m!=x){var d={position:m.css("position"),zIndex:m.css("zIndex")},g={position:"relative",zIndex:W+1};a.exposeElement=a.exposeElement||m;"object"===typeof a.expose&&$.each(a.expose,function(a,b){d[a]=m.css(a);g[a]=b});a.exposeElement.data("jTour",d).css(g).addClass(l+"exposed")}i?
