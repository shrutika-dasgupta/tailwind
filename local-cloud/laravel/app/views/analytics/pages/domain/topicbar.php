<?php
// Feature ACL.
$query_topics_max = $customer->maxAllowed('domain_query_topics_max');
?>

<div id="topic-bar" class="row-fluid hidden">
    <div class="span12">
        <div id="tags-dropdown" class="dropdown"><?= $tags ?></div>
        <form action="<?= URL::route('domain') ?>" method="POST" id="topic-bar-form">
            <?php $placeholder = ($query_topics_max == 1) ? 'Enter a Domain...' : 'Enter Domains...'; ?>
...
<div id="manageTopicsModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="manageTopicsModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <form id="add-topic-form" class="form-inline" action="<?= URL::route('domain-add-topic') ?>" method="POST">
...
<div id="tagTopicsModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="tagTopicsModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <form id="add-tag-form" action="<?= URL::route('domain-add-tag') ?>" method="POST">
            <div class="input-append">
                <input type        = "text"
                       name        = "name"
                       id          = "tag-name"
...
function loadTopicBar()
{
    $.ajax({
        type: 'GET',
        url: '<?= URL::route("domain-topic-bar") ?>',
        cache: false,
