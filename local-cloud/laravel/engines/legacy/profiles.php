...
try {
    ...
    if ($engine->running()) {
        ...
        if (count($calls) < 500) {
            ...
            $users = getUsersToPull(100, getFlatDate(time()), $conn);
...
function getUsersToPull($limit, $before_date, $conn) {
    $users = getUsersToPullPinsWithTrack($limit, "user", $before_date, $conn);

    if (count($users) < $limit) {
        $users = array_merge($users, getUsersToPullPinsWithTrack($limit - count($users), "competitor", $before_date, $conn));
    }

    if (count($users) < $limit) {
        $users = array_merge($users, getUsersToPullPinsWithTrack($limit - count($users), "keyword_trackin", $before_date, $conn));
    }

    if (count($users) < $limit) {
        $users = array_merge($users, getUsersToPullPinsWithTrack($limit - count($users), "free", $before_date, $conn));
    }

    if (count($users) < $limit) {
        $users = array_merge($users, getUsersToPullTrackingOnly($limit - count($users), "track", $before_date, $conn));
    }
    ...
    if (count($users) < $limit) {
        $users = array_merge($users, getUsersToPullTrackingOnly($limit - count($users), "", $before_date, $conn));
    }
...
function popUserAPICalls($limit, $conn) {
    ...
    if (count($calls) < $limit) {
        $calls = array_merge($calls, popAPICalls("User", "competitor", $limit - count($calls), false, $conn));
    }

    if (count($calls) < $limit) {
        $calls = array_merge($calls, popAPICalls("User", "free", $limit - count($calls), false, $conn));
    }

    if (count($calls) < $limit) {
        $calls = array_merge($calls, popAPICalls("User", "track", $limit - count($calls), false, $conn));
    }
    ...
    if (count($calls) < $limit) {
        $calls = array_merge($calls, popAPICalls("User", "keyword_trackin", $limit - count($calls), false, $conn));
    }
