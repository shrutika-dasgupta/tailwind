...
try {
    ...
    if($current_hour > 3){
        ...
        Log::debug('Grabbing User profiles to run calcs');
        $profiles = getProfilesForCalcs("user", getFlatDate(time()));
        if (count($profiles) == 0) {

            $current_track_type = 'competitor';
            Log::debug('No more User profiles.  Grabbing Competitor profiles.');
...
function calculateProfileFollowerDistribution($profiles) {
    ...
    foreach($profiles as $profile) {
        ...
        Log::debug("Calculated Follower Distribution ($user_id):: " . ($end - $start) . " seconds.");
    }
