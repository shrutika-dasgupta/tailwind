...
function getBrandsToPullPins($limit, $before_date, $conn) {
    $sources = getDomainsToPullPinsWithTrack($limit, "competitor", $before_date, $conn);
    ...
    if (count($sources) < $limit) {
        $sources = array_merge($sources, getDomainsToPullPinsWithTrack($limit - count($sources), "free", $before_date, $conn));
    }
...
function popBrandMentionsAPICalls($limit, $conn) {
    $calls = popAPICalls("Brand Mentions", "competitor", $limit, false, $conn);
    ...
    if (count($calls) < $limit) {
        $calls = array_merge($calls, popAPICalls("Brand Mentions", "free", $limit - count($calls), false, $conn));
    }
