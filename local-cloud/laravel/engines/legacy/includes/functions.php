...
function savePinLikeActivity($likes, $conn) {
    $queue_user_ids = array();

    Log::debug('Inserting pin-likes activity [legacy]');

    $inserts = "";
    foreach($likes as $like) {
        $pin_id = mysql_real_escape_string($like['pin_id']);
        $liker_user_id = mysql_real_escape_string($like['liker_user_id']);
        $follower_count = mysql_real_escape_string($like['follower_count']);
        $gender = mysql_real_escape_string($like['gender']);

        array_push($queue_user_ids, $liker_user_id);

        $timestamp = time();

        if ($inserts == "") {
            $inserts = "INSERT into data_pins_likes (pin_id, liker_user_id, follower_count, gender, timestamp) VALUES
				(\"$pin_id\", \"$liker_user_id\", \"$follower_count\", \"$gender\", \"$timestamp\")";
        } else {
            $inserts .= ",
				( \"$pin_id\", \"$liker_user_id\", \"$follower_count\", \"$gender\", \"$timestamp\")";
        }
...
function savePinRepinActivity($repins, $conn) {
    $queue_user_ids = array();

    $inserts = "";
    foreach($repins as $repin) {
        $repinner_user_id = mysql_real_escape_string($repin['repinner_user_id']);
        $pin_id = mysql_real_escape_string($repin['pin_id']);
        $board_id = mysql_real_escape_string($repin['board_id']);
        $board_url = mysql_real_escape_string($repin['board_url']);
        $board_name = mysql_real_escape_string($repin['board_name']);
        $category = mysql_real_escape_string($repin['category']);
        $board_follower_count = mysql_real_escape_string($repin['board_follower_count']);
        $is_collaborative = mysql_real_escape_string($repin['is_collaborative']);
        $created_at = mysql_real_escape_string($repin['created_at']);


        array_push($queue_user_ids, $repinner_user_id);

        $timestamp = time();

        if ($inserts == "") {
            $inserts = "INSERT into data_pins_repins (pin_id, repinner_user_id, board_id, board_url, board_name, category, board_follower_count, is_collaborative, created_at, timestamp) VALUES
				(\"$pin_id\", \"$repinner_user_id\", \"$board_id\", \"$board_url\", \"$board_name\", \"$category\", \"$board_follower_count\", \"$is_collaborative\", \"$created_at\", \"$timestamp\")";
        } else {
            $inserts .= ",
				(\"$pin_id\", \"$repinner_user_id\", \"$board_id\", \"$board_url\", \"$board_name\", \"$category\", \"$board_follower_count\", \"$is_collaborative\", \"$created_at\", \"$timestamp\")";
        }
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE board_id = VALUES(board_id), board_url = VALUES(board_url), board_name = VALUES(board_name), category = VALUES(category), board_follower_count = VALUES(board_follower_count), is_collaborative = VALUES(is_collaborative), created_at = VALUES(created_at), timestamp = VALUES(timestamp)";
        $acc_res = mysql_query($inserts,$conn) or die(mysql_error());
    }

    queueUserIds($queue_user_ids, $conn, "repin_user");
}

function savePinCommentActivity($comments, $conn) {
    $queue_user_ids = array();

    Log::debug('Saving pin-comment activity [legacy]');
    ...
    foreach($comments as $comment) {
        ...
        $created_at = mysql_real_escape_string($comment['created_at']);

        array_push($queue_user_ids, $commenter_user_id);

        $timestamp = time();

        if ($inserts == "") {
            $inserts = "INSERT into data_pins_comments (comment_id, pin_id, commenter_user_id, comment_text, created_at, timestamp) VALUES
				(\"$comment_id\", \"$pin_id\", \"$commenter_user_id\", \"$comment_text\", \"$created_at\", \"$timestamp\")";
        } else {
            $inserts .= ",
				(\"$comment_id\", \"$pin_id\", \"$commenter_user_id\", \"$comment_text\", \"$created_at\", \"$timestamp\")";
        }
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE pin_id = VALUES(pin_id), commenter_user_id = VALUES(commenter_user_id), comment_text = VALUES(comment_text), created_at = VALUES(created_at), timestamp = VALUES(timestamp)";
        $acc_res = mysql_query($inserts,$conn) or die(mysql_error());
    }
...
function saveBoards($boards, $conn) {
    ...
    Log::debug('Saving boards [legacy]');
    ...
    foreach($boards as $board) {
        ...
        array_push($queue_user_ids, $owner_user_id);
        ...
        $layout             = '';//mysql_real_escape_string($board->layout);
        $last_pulled        = time();
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE owner_user_id = VALUES(owner_user_id), url = VALUES(url), is_collaborator = VALUES(is_collaborator), is_owner = VALUES(is_owner), collaborator_count = VALUES(collaborator_count), image_cover_url = VALUES(image_cover_url), name = VALUES(name), description = VALUES(description), category = VALUES(category), pin_count = VALUES(pin_count), follower_count = VALUES(follower_count), created_at = VALUES(created_at), last_pulled = VALUES(last_pulled), track_type = VALUES(track_type), timestamp = VALUES(timestamp)";
        $acc_res = mysql_query($inserts,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    }

    Log::debug('Queuing user_ids from boards to be inserted [legacy]');
    queueUserIds($queue_user_ids, $conn);
}
...
function saveFollowers($followers, $conn) {
    ...
    Log::debug('Saving followers [legacy]');
    $inserts = "";
...
function saveUsers($users, $conn) {
    ...
    Log::debug('Inserting users [legacy]');
    ...
    foreach($users as $user) {
        ...
        $google_plus_url     = mysql_real_escape_string($user->google_plus_url);
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE username = VALUES(username), first_name = VALUES(first_name), last_name = VALUES(last_name), image = VALUES(image), about = VALUES(about), domain_url = VALUES(domain_url), domain_verified = VALUES(domain_verified), website_url = VALUES(website_url), created_at = VALUES(created_at), facebook_url = VALUES(facebook_url), twitter_url = VALUES(twitter_url), google_plus_url = VALUES(google_plus_url), location = VALUES(location), board_count = VALUES(board_count), pin_count = VALUES(pin_count), like_count = VALUES(like_count), follower_count = VALUES(follower_count), following_count = VALUES(following_count), last_pulled = VALUES(last_pulled), track_type=IF(VALUES(track_type)='user','user',IF(VALUES(track_type)='competitor', 'competitor', track_type)), p_gender = VALUES(p_gender), timestamp = VALUES(timestamp)";
        $acc_res = mysql_query($inserts,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    }
}
...
function savePins($pins, $conn) {
    ...
    Log::debug('Inserting pins into data_pins_new [legacy]');
    ...
    foreach($pins as $pin) {
        ...
        $origin_pin = mysql_real_escape_string($pin->origin_pin);
        $origin_pinner = mysql_real_escape_string($pin->origin_pinner);
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE user_id = VALUES(user_id), board_id = VALUES(board_id), domain = VALUES(domain), method = VALUES(method), is_repin = VALUES(is_repin), parent_pin = VALUES(parent_pin), via_pinner = VALUES(via_pinner), origin_pin = VALUES(origin_pin), origin_pinner = VALUES(origin_pinner), image_url = VALUES(image_url), image_square_url = VALUES(image_square_url), link = VALUES(link), description = VALUES(description), location = VALUES(location), dominant_color = VALUES(dominant_color), rich_product = VALUES(rich_product), repin_count = VALUES(repin_count), like_count = VALUES(like_count), comment_count = VALUES(comment_count), created_at = VALUES(created_at)";
        $acc_res = mysql_query($inserts,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    }

    Log::debug('Inserting pins into map_pins_descriptions [legacy]');
    savePinDescriptions($pins, $conn);

    Log::debug('Inserting pins into map_pins_attribution (if attribution exists) [legacy]');
    savePinAttributionData($pins, $conn);
    mapPinsToKeyword($pins, $conn);
    if(!isset($track_type)){
        $track_type = "track";
    }
    if ($track_type == "keyword") {

        Log::debug('Queuing user_ids from keyword pins to be inserted [legacy]');
        queueUserIds($queue_user_ids, $conn, 'keyword_tracking');
    } else {

        Log::debug('Queuing user_ids from brand mention pins to be inserted [legacy]');
        queueUserIds($queue_user_ids, $conn);
    }
}

function savePinAttributionData($pins, $conn) {
    $inserts = "";
    foreach($pins as $pin) {

        if ($pin->has_attribution) {
            $pin_id = mysql_real_escape_string($pin->id);
            $provider_name = mysql_real_escape_string($pin->attribution_provider_name);
            $author_name = mysql_real_escape_string($pin->attribution_author_name);
            $author_url = mysql_real_escape_string($pin->attribution_author_url);
            $title = mysql_real_escape_string($pin->attribution_title);
            $url = mysql_real_escape_string($pin->attribution_url);

            $time = time();
            if ($inserts == "") {
                $inserts = "INSERT into map_pins_attribution (
						pin_id
						, provider_name
						, author_name
						, author_url
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE provider_name = VALUES(provider_name), author_name = VALUES(author_name), author_url = VALUES(author_url), title = VALUES(title), url = VALUES(url)";
        $acc_res = mysql_query($inserts,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    }
}


function savePinDescriptions($pins, $conn) {
    $inserts = "";
    foreach($pins as $pin) {
        $pin_id = mysql_real_escape_string($pin->id);
        $description = substr(mysql_real_escape_string($pin->description), 0, 500);
        $created_at = mysql_real_escape_string($pin->created_at);

        if ($inserts == "") {
            $inserts = "INSERT IGNORE into map_pins_descriptions (
					pin_id
					, description
					, created_at)
					VALUES (
...
function mapPinsToKeyword($pins, $conn) {
    $inserts = "";
    foreach($pins as $pin) {
        if ($pin->keyword != "") {
            $pin_id = mysql_real_escape_string($pin->id);
            $keyword = mysql_real_escape_string($pin->keyword);
            $pinner_id = mysql_real_escape_string($pin->user_id);
            $domain = mysql_real_escape_string($pin->domain);
            $created_at = mysql_real_escape_string($pin->created_at);
            ...
            if ($inserts == "") {
                $inserts = "INSERT into map_pins_keywords (
						pin_id
						, keyword
						, pinner_id
						, domain
    ...
    if ($inserts != "") {
        $inserts .= "
			ON DUPLICATE KEY UPDATE pinner_id = VALUES(pinner_id), domain = VALUES(domain), created_at = VALUES(created_at)";
        $acc_res = mysql_query($inserts,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    }
}

function queueUserIds($queue_user_ids, $conn, $track_type = "track") {
    $inserts = "";

    foreach($queue_user_ids as $user_id) {
        $user_id = mysql_real_escape_string($user_id);
        if ((!$track_type) || ($track_type == "")) {
            $track_type = "track";
        }
        $time = time();
...
function getAPIDataFromCall($data) {
    if (array_key_exists("data", $data)) {
        Log::debug('Getting data from the API Call [legacy]');
        return $data['data'];
    } else {
        Log::notice('No data found returning an empty array [legacy]');
        return array();
    }
}
...
function isValidAPIReturn($data, $call, $conn) {
    ...
    if (is_array($call)) {
        if (array_key_exists("api_call", $call)) {
            $call_name = mysql_real_escape_string($call['api_call']);
        } else {
            $call_name = "";
        }

        if (array_key_exists("object_id", $call)) {
    ...
    if (is_array($data)) {
        ...
        if (array_key_exists("message_detail", $data)) {
            $message_detail = mysql_real_escape_string($data['message_detail']);
        } else {
            $message_detail = "";
        }
    } else {
    ...
    $timestamp = time();
    $acc = "INSERT into status_api_errors VALUES (\"\", \"$call_name\", \"$object_id\", '', \"$message\", \"$message_detail\", \"$code\", \"$timestamp\")";
    $acc_res = mysql_query($acc,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());

    if ($code == 10) {
        removeAPICall($call, $conn);
    }
...
function popAPICalls($api_call, $track_type, $limit, $remove, $conn) {
    if (!$limit) {
        $limit = 1;
    }
    Log::debug('Finding api calls to pull off the queue [legacy]');
    $calls = array();
    ...
    Log::debug('Pulling api calls off the queue [legacy]');
    ...
    foreach($calls as $call) {
        ...
        if ($remove) {
            ...
            $time = time();
            $acc = "update status_api_calls_queue set timestamp = '$time' where api_call = '$api_call' AND object_id = '$object_id' AND parameters = '$parameters' AND bookmark = '$bookmark'";
            $acc_res = mysql_query($acc,$conn) or die('functions.pjp line '.__LINE__.': '.mysql_error());
        }
    }
...
function removeAPICall($call, $conn) {
    $api_call = mysql_real_escape_string($call['api_call']);
    $object_id = mysql_real_escape_string($call['object_id']);
    $parameters = mysql_real_escape_string($call['parameters']);
    $bookmark = mysql_real_escape_string($call['bookmark']);

    Log::debug('Removing API Call [legacy]');
...
function queueAPICall($api_call, $object_id, $parameters, $bookmark, $track_type, $conn) {
    ...
    Log::debug("Queing up new API call [legacy] - $api_call");
    ...
    mysql_query($sql,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
}
...
function getEngineStatusReset($task, $conn) {
    $acc = "select * from status_engines where engine='$task'";
    $acc_res = mysql_query($acc,$conn) or die('functions.php line '.__LINE__.': '.mysql_error());
    while ($a = mysql_fetch_array($acc_res)) {
        $status = $a['status'];
...
function setEngineStatus($task, $status, $conn) {
    ...
    $time = time();
    $sql = "INSERT into status_engines (engine, status, timestamp)
            values ('$task','$status','$time')
            ON DUPLICATE KEY UPDATE
