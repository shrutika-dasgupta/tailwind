...
{
	private $api_key = '-Qf_c1QzLpOTom7bzQZJ';
	private $test_api_key = '-Qf_c1QzLpOTom7bzQZJ';
	private $domain = 'tailwind'; //your chargify domain, e.g. if you have [your-domain].chargify.com, then enter "your-domain" only.
	private $test_domain = 'tailwind';
    ...
    public function getStatementByID($id, $format = 'XML')
    {
        $base_url = "/statements/$id." . strtolower($format);

        $request = $this->sendRequest($base_url, $format);

        if ($format == 'XML') {
            return new SimpleXMLElement($request->response);
        }

        return $request->response;
    }

    public function getStatementsBySubscriptionID($id)
    {
        $base_url  = "/subscriptions/$id/statements.xml";
        $base_url .= '?sort=closed_at&direction=desc';

        $request = $this->sendRequest($base_url);
    ...
    public function retrieveComponentByComponentId($product_family_id, $component_id, $format = 'XML') {
        $extension = strtoupper($format) == 'XML' ? '.xml' : '.json';
        $base_url = "/product_families/{$product_family_id}/components/{$component_id}" . $extension;
